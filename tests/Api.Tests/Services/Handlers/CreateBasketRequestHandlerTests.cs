...
{
    public class CreateBasketRequestHandlerTests
    {
        [Fact]
        public async Task Handle_ShouldReturnBasket()
        {
            ...
            var handler = new CreateBasketRequestHandler(mapper.Object, store.Object);

            var basket = await handler.Handle(new CreateBasketRequest());
